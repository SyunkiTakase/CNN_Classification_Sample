...
def train(device, train_loader, model, criterion, optimizer, scaler, use_amp, epoch):
    model.train()
...
def saves_train(device, train_loader, model, criterion, optimizer, scaler, use_amp, epoch):
    model.train()
    ...
    for img,label in tqdm(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            logit = model(img_mixup)
            loss = criterion(logit, label_mixup)
